...
from flask import Flask, request, render_template, Response, stream_with_context, stream_template
import cv2
import numpy as np
...
@app.route('/index2')
def index2():
    rows = generate_frames()
    return Response(stream_with_context(stream_template('index2.html', rows=rows)))
